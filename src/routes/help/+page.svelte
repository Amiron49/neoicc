<script lang="ts">
	...
	const changelog = [
		{
			date: '03.03.2021',
			changes: ['Small fix for the download image button in the viewer, should work now.']
		},
		{
			date: '28.02.2021',
			changes: [
		...
		},
		{
			date: '30.01.2021',
			changes: [
				"Force and unselect functions on choices can now use the ID's of other choices like this: ID,ID,ID/ON#2 (No spaces).",
	...
	const tips = [
		...
		},
		{
			...
			content: [
				...
				'Cutting down to 40ish percent quality will make little difference in most cases, but shave off up to 60-70 percent of the image file size.'
			]
		}
	];
</script>
...
<main class="prose mx-auto mt-8">
	<div class="flex flex-row items-center gap-x-4">
		...
		<h1 class="mb-0">Information</h1>
	</div>
	<Accordion.Root type="single" class="not-prose mt-4 w-full">
		<Accordion.Item class="rounded-t border">
			<Accordion.Trigger class="rounded-t p-2">CHANGELOG</Accordion.Trigger>
			<Accordion.Content>
				<div class="flex flex-col gap-y-2 border-t p-2">
					{#each changelog as { date, changes } (date)}
						<Accordion.Root type="single" class="not-prose w-full">
							<Accordion.Item class="rounded border">
								<Accordion.Trigger class="rounded-t p-2">{date}</Accordion.Trigger>
								<Accordion.Content>
									<div class="flex flex-col gap-y-2 border-t p-4">
		...
		<Accordion.Item class="border-x">
			<Accordion.Trigger class="rounded-t p-2">How do I show off my CYOA?</Accordion.Trigger>
			<Accordion.Content>
				<div class="flex flex-col gap-y-2 border-t p-2">
					<p>1. Host it yourself on a free hosting service.</p>
					<p>Either</p>
					<p>
						A. Download the Viewer from the link below, open the JSON file of the project and the
						app.XXXXXXX.js file in notepad. Then copy all from your project and place it in the gap
						between
						<b>
							{'{'}state:{'{'}app:
						</b>{' '}
						and <b>{'}'},getters:</b> near the bottom of the smallest .js file in the js folder.
					</p>
					<p>OR</p>
					<p>
						B. Download the Viewer from the link below, get your project file, make sure the project
						file is named 'project', place it next to the index.html file in the Viewer. If you do
						it this way then it will not work unless it's uploaded onto a hosting service, but when
						its there all you need is to replace the project file to update your project, it's the
						better solution.
					</p>
					<p>Then</p>
					<p>
						Create a user on Neocities or another free hosting service, move to the 'Edit your page'
						part of the site and upload the Viewer, anyone that enters the page will now see the
						Cyoa.
					</p>
					<p>
					...
					<p>
						Upload it to Mega or some other site, and let people download it and open it in the
						creator themselves.
					</p>
				</div>
			</Accordion.Content>
		...
		<Accordion.Item class="border-x">
			<Accordion.Trigger class="rounded-t p-2">
			...
			<Accordion.Content>
				<div class="flex flex-col gap-y-2 border-t p-2">
					<p>
						Yes, yes you can!
						<br />
						In the top menu of the viewer in the creator, you can see a button named 'Download image',
						press this and a download will start. The bigger your project is the longer it will take
						before the download starts, and the bigger the file will be.
					</p>
					<p>
						<br />
						Make them big enough and pictures near the bottom will not appear on the image, if this happens
						you should temporarily delete some rows and split your project into multiple pictures.
					</p>
					<br />
					<p>
						If your project is very large, then it might be wiser for you to download an extension
						with more options, like
						<a href="https://www.awesomescreenshot.com">Awesome Screenshot</a>
						which you can use to download your project as PDF or only capture a part of it.
					</p>
				</div>
			</Accordion.Content>
		</Accordion.Item>
		<Accordion.Item class="border-x border-b">
			<Accordion.Trigger class="rounded-t p-2">
				How do I use the Interactive CYOA Creator?
			</Accordion.Trigger>
			<Accordion.Content>
				<div class="flex flex-col gap-y-2 border-t p-2">
					{#each tips as { title, content } (title)}
						<Accordion.Root type="single" class="not-prose w-full">
							<Accordion.Item class="rounded border">
								<Accordion.Trigger class="rounded-t p-2">{title}</Accordion.Trigger>
								<Accordion.Content>
									<div class="flex flex-col gap-y-2 border-t p-4">
										{#each content as tip, index (index)}
											<!-- eslint-disable-next-line svelte/no-at-html-tags -->
											<p>{@html tip}</p>
										{/each}
									</div>
